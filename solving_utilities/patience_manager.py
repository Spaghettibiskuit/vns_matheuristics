"""Class that manages the patience per subproblem for a for each number of zones."""

import itertools


class PatienceManager:
    """Manages the patience per subproblem for for each number of zones.
    ...
    equal amount of time per number of zones. I.e. if 1 minute is spent per subproblem with 4
    zones, 2 minutes are spent per subproblem with 3 zones.

    How this idea is mapped to the concept of patience:
    If we replace the time spent per subproblem with patience there is the benefit of staying in
    ...
    Further, it is always made sure that the sum of patiences over all pairs is non-decreasing as
    the number of zones gets lower. E.g. if the patience per pair was 1 minute for 4 zones is
    increased by a step of 10 seconds, the sum of patience is now 7 minutes. Then the sum of
    patiences for 3 zones must be at least 7 minutes, i.e. 2 minutes and 20 seconds per pair.
    With every step to the patience for any of the number of zones, the patiences for lower numbers
